...
public class BearerTokenServerAuthenticationConverter implements ServerAuthenticationConverter {
    ...
    //пользователь обращается и попалет на фильтр, используем BearerTokenServerAuthenticationConverter, он вытаскивает из запроса header
    //затеим уюирает "Bearer ", получает токен, токен уходит на проверку check, после check создаем Authentication и закидваем его в MONO
    @Override
